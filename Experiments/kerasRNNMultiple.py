...
def downsample(datasetorigin, binSize):
	downsampleDataset = []
	dataset = np.transpose(datasetorigin)
	#print(len(dataset))
	j=0
...
dataset = []
for i in range(10):
	data = np.genfromtxt('./Spike Results/'+str(i+1)+'idTimes.csv', delimiter = ',')
...
model.add(keras.layers.LSTM(10,input_shape=(100, popSize),return_sequences=True))
...
trainScore = math.sqrt(log_loss(testY, testPredict))
print('Non-thresholded: %.2f RMSE' % (trainScore))
testScore = math.sqrt(log_loss(testY, prediction.transpose()))
